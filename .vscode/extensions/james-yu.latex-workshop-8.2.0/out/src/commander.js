...
class Commander {
    ...
    async view(mode) {
        ...
        const configuration = vscode.workspace.getConfiguration('latex-workshop');
        const useActiveGroup = configuration.get('view.pdf.tab.useNewGroup');
        if (mode === 'browser') {
    ...
    */
    ...
    shiftSectioningLevel(change) {
        ...
        function replacer(_match, sectionName, options, contents) {
            if (change === 'increment') {
    ...
    devParseTeX() {
        ...
        const ast = latex_utensils_1.latexParser.parse(vscode.window.activeTextEditor.document.getText());
    ...
    devParseBib() {
        ...
        const ast = latex_utensils_1.bibtexParser.parse(vscode.window.activeTextEditor.document.getText());
        vscode.workspace.openTextDocument({ content: JSON.stringify(ast, null, 2), language: 'json' }).then(doc => vscode.window.showTextDocument(doc));
    }
    texdoc(pkg) {
