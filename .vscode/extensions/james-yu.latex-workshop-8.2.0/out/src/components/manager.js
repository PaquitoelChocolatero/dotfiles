...
class Manager {
    ...
    getOutDir(texPath) {
        ...
        const out = configuration.get('latex.outDir');
    ...
    getDirtyContent(file, reload = false) {
    ...
    parseInputFiles(content, baseFile) {
        const inputReg = /(?:\\(?:input|InputIfFileExists|include|subfile|(?:(?:sub)?(?:import|inputfrom|includefrom)\*?{([^}]*)}))(?:\[[^[\]{}]*\])?){([^}]*)}/g;
        while (true) {
            const result = inputReg.exec(content);
            if (!result) {
    ...
    parseInputFilePath(regResult, baseFile) {
    ...
    parseBibFiles(content, baseFile) {
    ...
    parseFlsContent(content, flsFile) {
    ...
    createFileWatcher() {
        ...
        if (this.fileWatcher) {
            this.fileWatcher.on('add', (file) => this.onWatchingNewFile(file));
            this.fileWatcher.on('change', (file) => this.onWatchedFileChanged(file));
            this.fileWatcher.on('unlink', (file) => this.onWatchedFileDeleted(file));
        }
    ...
    async updateCompleter(file, content) {
        ...
        // the keypress delay.
        const latexAst = await this.extension.pegParser.parseLatex(content, { timeout: configuration.get('intellisense.update.delay', 1000) });
        if (latexAst) {
    ...
    resolveBibPath(bib, rootDir) {
        const bibDirs = vscode.workspace.getConfiguration('latex-workshop').get('latex.bibDirs');
