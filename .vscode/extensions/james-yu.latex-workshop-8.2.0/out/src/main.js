...
function renameValue(config, oldValue, newValue) {
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.languages.registerFoldingRangeProvider({ scheme: 'file', language: 'latex' }, new folding_1.FoldingProvider(extension)));
    extension.manager.findRoot();
    extension.linter.lintRootFileIfEnabled();
    obsoleteConfigCheck(extension);
    ...
    return {
        ...
        viewer: {
            ...
            openTab: (sourceFile, respectOutDir = true, sideColumn = true) => extension.viewer.openTab(sourceFile, respectOutDir, sideColumn)
        },
        manager: {
